<?php 
	...
	if ($security && !empty($u_id)) {
		...
		if ($u_level == "admin" && $type == "search") {
			...
			if (!empty($patient_id)) {
				if ($sqlWhere != "") { $sqlWhere .= " AND "; }
				$sqlWhere .= "p.patient_id = '".$patient_id."'";
			}
			/*
			if (!empty($u_id)) {
				if ($sqlWhere != "") { $sqlWhere .= " AND "; }
		...
		$sql = "SELECT * FROM patients p ".
				"INNER JOIN patient_title pt ON pt.patient_title_id = p.patient_title_id ".$sqlWhere;
		...
		while ($rs = fetch_array($res)) {
			...
			$data["patient_birthdate"] = isset($rs["patient_birthdate"]) ? dateToPage($rs["patient_birthdate"]) : "";
