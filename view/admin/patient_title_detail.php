<?php 
	...
	$patient_title_id = isset($_GET["p"]) ? $_GET["p"] : "";
		...
		<div class="main-wrapper main-wrapper-1">
			...
			<div class="main-content">
				<section class="section">
					...
					<a href="patient_title.php"><button type="button" class="btn btn-danger m-3">กลับไปหน้าคำนำหน้า</button></a>
													...
													<div class="row mt-5">
														<div class="col-12">
															<div class="form-group m-0">
																<label for="patient_title_name">คำนำหน้า</label>
																<input id="patient_title_name" type="text" class="form-control" name="patient_title_name" value="" required>
															</div>
														</div>
													</div>
...
<script>
	let patient_title_id = '<?php echo $patient_title_id; ?>';
	...
	function loadData() {
		...
		if (patient_title_id != "") {
			$.ajax({
				...
				data: { 'type':'id', 'patient_title_id':patient_title_id },
				dataType: "json",
				beforeSend: function() {},
				success: function (res) {
					...
					if (row.length > 0) {
						row.map( e => {
							...
							$('#patient_title_name').val(e.patient_title_name);
	...
	function saveData() {
		...
		if (status) {
			...
			*/
			...
			let patient_title_name = $('#patient_title_name').val();
			


			if (patient_title_id != "" || patient_title_id == 0) {
				$.ajax({
					type: "POST",
					...
					data: {'type':'save', 'patient_title_id':patient_title_id,
					...
					success: function(data) {
						if(data.status){
							Swal.fire({
								...
								if (result.value) {
									window.location.href = '<?php echo base_url(); ?>view/admin/patient_title.php';
								}
							});
						} else {
							Swal.fire({
	...
	function checkData() {
		...
		if (patient_title_name != "" && patient_title_name != "" ) {
